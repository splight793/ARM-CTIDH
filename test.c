...
#include "csidh.h"
#include "primes.h"
//#define _BENCH_IN_CYCLE_COUNT_
#ifdef _BENCH_IN_CYCLE_COUNT_
const char unit[]="cycle(s)";
#else
const char unit[] = "ns";
#endif
uint32_t cpucycles(void)
{ // Access system counter for benchmarking
#ifndef _BENCH_IN_CYCLE_COUNT_
...
#ifdef NDEBUG
#pragma message "assert is discarded"
#endif
...
for(int i = 0; i < 8; i++) printf("0x%016llx ,\n", k->x.c[i]);
printf("----------\n");
}
void test_sqrt(void)
{
	initPredicate();
fp a ={1}, b = {0}, c={0xf2a21dd1f8c4fcdb ,
0xa8aa97352e692f97 ,
0xccc55bf43cb83d07 ,
0xf4c7a4454cf09f6a ,
0x1ec5920ee0cb1d89 ,
...
//initPredicate();
    ...
    fp_neg2(&xneg,&x);
    fp_neg2(&x2neg,&x2);
    //pp(&x2);pp(&xneg);pp(&x2neg);
    long vv[] = {fp_sqrt(&x), fp_sqrt(&xneg), fp_sqrt(&x2), !fp_sqrt(&x2neg)};
    if(((vv[0]==vv[1] )|| !vv[2])||!vv[3]) {
    //if(true){
    printf("%ld %ld %ld %ld\n", vv[0],vv[1],vv[2],vv[3]);
    pp(&c); pp(&x2);
    pp(&x2);pp(&xneg);pp(&x2neg);break;
    }
    fp_random(&c);
...
void privkey(private_key* k) {
for(int i = 0; i < primes_num; i++) {printf("%02x", k->e[i]);}
printf("\n");
}
void pubkey(public_key*k){
for(int i = 7; i >= 0; i--) printf("%016llx", k->A.x.c[i]);
printf("\n");
}
...
#define BENCH_LOOP 1000
void bench_nike() {
    private_key alice_priv;
    public_key alice_pub = base, shared_alice;
    uint32_t start, end;
    int64_t cycles = 0;
    for(int i = 0; i < BENCH_LOOP; i++)
    {
        start = cpucycles();
        csidh_private(&alice_priv);
        //validate(&base);
	end = cpucycles();
        cycles = cycles + (end - start);
    }
    printf("Key generation runs in..............................%10lld %s\n", cycles/BENCH_LOOP, unit);
    cycles = 0;
    for(int i = 0; i < BENCH_LOOP; i++)
    {
        start = cpucycles();
        csidh(&shared_alice, &alice_pub, &alice_priv);
        end = cpucycles() - start;
        cycles += end;
    }
    printf("Shared Key generation runs in..............................%10lld %s\n", cycles/BENCH_LOOP, unit);
